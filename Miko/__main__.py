...
import Miko.modules.sql.users_sql as sql
...
from Miko.modules import ALL_MODULES
from Miko.modules.disable import DisableAbleCommandHandler
from Miko.modules.helper_funcs.alternate import typing_action
from Miko.modules.helper_funcs.chat_status import is_user_admin
from Miko.modules.helper_funcs.misc import paginate_modules
...
PM_START_TEXT = """
✨ *ɪ ᴀᴍ ᴍɪᴋᴏ, ᴀ ɢᴇɴꜱʜɪɴ ɪᴍᴘᴀᴄᴛ ᴛʜᴇᴍᴇᴅ ʀᴏʙᴏᴛ ᴡʜɪᴄʜ ᴄᴀɴ ʜᴇʟᴘ ʏᴏᴜ ᴛᴏ ᴍᴀɴᴀɢᴇ ᴀɴᴅ ꜱᴇᴄᴜʀᴇ ʏᴏᴜʀ ɢʀᴏᴜᴘ ᴡɪᴛʜ ʜᴜɢᴇ ɢʀᴏᴜᴘ ᴍᴀɴᴀɢᴇᴍᴇɴᴛ*
"""
...
buttons = [
    ...
    [
        ...
        InlineKeyboardButton(text="SUPPORT", url=f"https://t.me/hunterXsupport"),
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module(f"Miko.modules.{module_name}")
...
def gitsource_callback(update, context):
...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
        if len(args) >= 1:
                ...
                send_help(
                    ...
                    InlineKeyboardMarkup(
                        [
                            [
                                InlineKeyboardButton(
                                    text="⇦ 𝘽𝘼𝘾𝙆", callback_data="help_back"
                                )
                            ]
                        ]
                    ),
            ...
            lol = update.effective_message.reply_photo(
                photo=str(choice(START_IMG)),
                caption=PM_START_TEX.format(usr.first_name),
                parse_mode=ParseMode.MARKDOWN,
            )
            time.sleep(0.2)
            guu = update.effective_message.reply_text("🐾")
            time.sleep(2)
            guu.delete()
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton(
                            text="UPDATES",
                            url=f"https://t.me/Hunter_Updates",
                        ),
                    ],
                    [
                        InlineKeyboardButton(
...
def Miko_callback_data(update, context):
    query = update.callback_query
    uptime = get_readable_time((time.time() - StartTime))
    if query.data == "Miko_":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(text="◁", callback_data="Miko_prev"),
                        InlineKeyboardButton(text="𝘽𝘼𝘾𝙆", callback_data="Miko_back"),
                        InlineKeyboardButton(text="▷", callback_data="Miko_next"),
                    ]
                ]
            ),
        )
...
def donate(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != 5907205317 and DONATION_LINK:
            update.effective_message.reply_text(
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.sendAnimation(
                f"@{SUPPORT_CHAT}",
                animation="https://telegra.ph/file/4fd1002d4e614e0c2e405.mp4",
                caption=f"""
...
""",
    ...
    gitsource_callback_handler = CallbackQueryHandler(
        gitsource_callback, pattern=r"git_source", run_async=True
    )

    data_callback_handler = CallbackQueryHandler(
        Miko_callback_data, pattern=r"Miko_", run_async=True
    )
    ...
    dispatcher.add_handler(CommandHandler("repo", repo))
    ...
    dispatcher.add_handler(gitsource_callback_handler)
